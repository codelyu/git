...
<body class="childrenBody">
		...
		<form class="layui-form" id="searchForm" method="post">
			...
			<div class="layui-form-item" style="text-align: center;">
				...
				<a class="layui-btn layui-btn-normal addInport_btn">添加</a>
...
layui.use(['form','layer','table','laydate'],function(){
    ...
    //商品列表
     tableIns = table.render({
        elem: '#inportList',
        url : '${pageContext.request.contextPath }/inport/queryAllInports.action',
        cellMinWidth : 95,
        page : true,
        height : "full-200",
        limits : [10,15,20,25],
    ...
    $(".search_btn").on("click",function(){
    	...
    	table.reload('inportListTable', {
    		  url: '${pageContext.request.contextPath }/inport/queryAllInports.action?'+params
    	});
    });
   
    //添加商品
    $(".addInport_btn").click(function(){
        var index = layui.layer.open({
            title : "添加商品",
            type : 2,//ifream层
            area:["700px","500px"],
            content : "${pageContext.request.contextPath }/inport/toInportAdd.action",
    ...
    function updateInport(data){
    	var index = layui.layer.open({
            ...
            type : 2,
            area:["700px","500px"],
            content : "${pageContext.request.contextPath }/inport/toUpdateInport.action?id="+data.id,
            success : function(layero, index){
                setTimeout(function(){
                    layui.layer.tips('点击此处返回商品列表', '.layui-layer-setwin .layui-layer-close', {
                        tips: 3
    ...
    table.on('tool(inportList)', function(obj){
        ...
        if(layEvent === 'edit'){ //编辑
            ...
            layer.confirm('确定删【'+data.inportname+'】商品吗？',{icon:3, title:'提示信息'},function(index){
                 $.post("${pageContext.request.contextPath }/inport/deleteInport.action",{
    ...
    $(".delAll_btn").click(function(){
        ...
        if(data.length > 0) {
            ...
            layer.confirm('确定删除选中的商品？', {icon: 3, title: '提示信息'}, function (index) {
                 $.post("${pageContext.request.contextPath }/inport/batchDeleteInport.action?"+ids,function(data){
                	 //刷新table
                     tableIns.reload();
                 	 //关闭提示框
                	layer.close(index);
